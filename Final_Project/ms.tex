...
which we report in \autoref{tab:priors}.
\EW{I'm assuming this should be the \Planck priors table?}
...
\label{sec:rest}
\EW{Should we mention the issue with the covariance matrix in this section or the next? Or not at all?}
...
\EW{Added \textbf{we assume} to the last sentence because $\Lambda$CDM and $w$CDM do not necessarily assume a spatially flat universe}
...
\end{equation}
%
\EW{Is this equation missing the normalization constant?}
\TM{No it is general. The equation before is normalized properly, and this one 
  doesn't specify the prior, so it is normalized in that the prior hasn't been
  specified yet.}
...
in \autoref{fig:betoule_lcdm_corner} and \autoref{fig:rest_lcdm_corner}. 

Similary, \autoref{tab:wcdm}, \autoref{fig:betoule_wcdm_corner}, and \autoref{fig:rest_wcdm_corner}
detail the most likely values and confidence intervals of $\Omega_M$, $h$, and $w$ 
in a $w$CDM model. An extra entry is given in \autoref{tab:wcdm} for just our prior, the 
\Planck likelihood. 
\LC{Not done yet}
