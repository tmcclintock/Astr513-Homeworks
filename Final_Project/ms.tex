...
\end{equation} where the Hubble constant $H_0$ can be written in terms of a 
...
\begin{equation}
  \label{eq:likelihood}
  P(\mu(z)|\mathrm{cos}) = \frac{1}{\sqrt{2\pi|\mathbf{C}|}}\exp
  \left[-\frac{1}{2}\sum_{i,j=0}^{N_{samples}}\left(\mu(z) - \mu_{\mathrm{model}}(z)\right)_i\mathbf{C}^{-1}_{i,j}\left(\mu(z) - \mu_{\mathrm{model}}(z)\right)_j\right]
\end{equation}
%
where $\mathbf{C}$ is the full covariance matrix between all of the $N_{samples}$ 
in the data, and $\mathrm{cos}$ represents the cosmology that we are investigating
...
\begin{equation}
  \label{eq:posterior}
  P(\mathrm{cos}|\mu(z)) = P_{prior}(\mathrm{cos}) P(\mu(z)|\mathrm{cos}).
\end{equation}
%
...
detail for each parameter in Table~XXX
\TM{Make this table. Just look at the cosmology.ini files.}
