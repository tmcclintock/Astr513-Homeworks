...
all parameters simultaneously in addition to \Planck priors on all parameters.
...
\begin{figure}
  \centering
  \includegraphics[width=0.48\linewidth]{figures/betoule_wcdm_withprior_bestfit.eps}
  \includegraphics[width=0.48\linewidth]{figures/rest_wcdm_withprior_bestfit.eps}
  \caption{{\it Left}: best fit model of the \citet{betoule2014} data for a $w$CDM model 
     with \Planck priors. {\it Right}: best fit model of the \citet{rest2014} data for a $w$CDM model 
     with \Planck priors. Blue points are the data with error bars corresponding 
     to the diagonal of the provided covariance martrix. The solid black line is 
     the most likely model from our MCMC chain.}
  \label{fig:wcdm_prior_bestfit}
\end{figure}

\section{Conclusions}
\label{sec:conclusions}
\LC{Not sure if I should say anything more about \autoref {fig:wcdm_prior_bestfit} here.}
As seen in \autoref{tab:lcdm} and \autoref{fig:betoule_lcdm_corner}, the addition of the \Planck priors
to the \citet{betoule2014} data tightens the constraints on $\Omega_M$ and $h$ in a $\Lambda$CDM model. 
Moreover, the \Planck priors vastly improve the constraint on $\Omega_M$ despite the issues in \citet{rest2014} data
as demonstrated in \autoref{fig:rest_lcdm_corner}. 

Comparably, the inclusion of the \Planck priors to the \citet{betoule2014} data also tighten the 
constraints on $\Omega_M$, $h$, and $w$ in a $w$CDM model as seen in \autoref{tab:wcdm} and  
\autoref{fig:betoule_wcdm_corner}. This also applies to the \citet{rest2014} data as illustrated 
in \autoref{fig:rest_wcdm_corner}, where $\Omega_M$ and $w$ are well constrained compared to the case with a flat prior. 
The confidence contours and most likely values are nearly identical to the ones from the \Planck likelihood. 
